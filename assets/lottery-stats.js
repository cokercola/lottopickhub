/* lottery-stats.js
 * Generic stats module shared across every lottery game page
 * (Powerball, Mega Millions, and future games). Reads a game-specific
 * history JSON file (every draw since that game's current-matrix
 * cutoff date) and computes hot/cold numbers, a frequency heatmap,
 * pair/triple analysis, a historical draw explorer, and a per-number
 * lookup - all client-side. Small history sizes (dozens to low
 * thousands of draws) mean every computation here runs in a few
 * milliseconds, so nothing is precomputed server-side.
 *
...
async function fetchLotteryHistory(historyPath) {
...
function initLookup(containerId, draws, ballLabel) {
  const el = document.getElementById(containerId);
  if (!el) return;
  ...
  function renderNumber(number) {
    ...
    resultEl.innerHTML = `
      ...
      <div class="lookup-grid">
        ...
        <div class="lookup-stat"><span class="lookup-label">Common ${ballLabel}</span><span class="lookup-value">${stats.commonPB ?? '-'}</span></div>
      </div>
  ...
  window.lotteryJumpToNumber = function jumpToNumber(number) {
    input.value = number;
    renderNumber(number);
...
async function initLotteryStats(ids, historyPath, specialBallLabel) {
  const history = await fetchLotteryHistory(historyPath);
  ...
  renderShared();
  if (ids.lookup) initLookup(ids.lookup, draws, ballLabel);
